...
export class LancamentoService {
  ...
  buscarUltimoTipoLancado(): Observable<any> {
    return this.http.get(env.baseUrlApi + this.PATH +
      this.PATH_ULTIMO_LANC.replace('{funcionarioId}', this.httpUtil.obterIdUsuario()), this.httpUtil.headers());
  }
  cadastrar(lancamento: Lancamento): Observable<any> {
    return this.http.post(env.baseUrlApi + this.PATH, lancamento, this.httpUtil.headers());
  }
